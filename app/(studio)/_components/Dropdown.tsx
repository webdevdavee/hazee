...
export default function Dropdown<T>({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute z-10 w-full mt-1 bg-base border rounded-md shadow-lg transition-all duration-200 ease-in-out ${
          isOpen
          ...
          {items && items.length > 0 ? (
            items.map((item, index) => (
              <li
                ...
                className="px-4 py-2 text-sm text-secondary hover:bg-blue-50 cursor-pointer transition-colors duration-150"
              >
                {renderItem(item)}
              </li>
            ))
